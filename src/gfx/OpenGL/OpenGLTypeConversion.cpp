...
const char* Gfx::getErrorCode(GLenum error)
{
	...
	{
		const ErrorDescription& description = errorDescriptionLUT[i];
...
const char* Gfx::getErrorDescription(GLenum error)
{
	for (size_t i = 0; i < sizeof(errorDescriptionLUT) / sizeof(errorDescriptionLUT[0]); ++i)
	{
		const ErrorDescription& errorDescription = errorDescriptionLUT[i];
		if (errorDescription.error == error)
		{
...
const DebugType Gfx::debugTypeLUT[] = {
...
const char* Gfx::getDebugType(GLenum type)
{
	for (size_t i = 0; i < sizeof(debugTypeLUT) / sizeof(debugTypeLUT[0]); ++i)
	{
		if (debugTypeLUT[i].type == type)
		{
			return debugTypeLUT[i].typeString;
		}
	}

	return "Unkown debug type";
}
#endif // DEBUG
